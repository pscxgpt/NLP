...
% Ajusta la tabla exactamente al ancho del texto en el PDF
\resizebox{\textwidth}{!}{%
% Definimos la primera columna con ancho fijo de 5cm y las otras dos centradas (c)
\begin{tabular}{lp{4.5cm}c}
\toprule
\textbf{Hyperparameter} & \textbf{Baseline Model} & \textbf{Advanced Optimized Model} \\
\midrule
Pre-trained Backbone & \texttt{roberta-base-} \newline \texttt{biomedical-clinical-es} & \texttt{roberta-base-} \newline \texttt{biomedical-clinical-es} \\
...
\label{tab:results}
% Ajusta la tabla automáticamente para que quepa exactamente entre los márgenes de la página
\resizebox{\textwidth}{!}{%
% Usamos p{6.5cm} para que los nombres largos de los modelos salten de línea limpiamente
\begin{tabular}{lp{0.5cm}cccc}
\toprule
